...
const statusLabels: Record<string, string> = {
  ...
  SCHEDULED: 'Agendado', PROCESSING: 'Processando', PROCESSED: 'Processado', CAPPED_250_PERCENT: 'Teto de 250% da cota atingido',
}
...
function ActivationOnboarding({ session }: { session: Session }) {
  const api = useApi(session)
  const [planCheckoutKey, setPlanCheckoutKey] = useState(() => crypto.randomUUID())
  ...
  const activateAssociate = async (event: FormEvent) => {
    ...
    try {
      if (planPaymentAsset === 'BALANCE') { await api.post('/wallet/purchases', { productType: 'ASSOCIATE_PLAN', idempotencyKey: planCheckoutKey }); location.reload(); return }
      ...
      setCheckout(result)
    } catch (reason: any) { setError(reason.message); if (reason instanceof ApiError && reason.retryable) setPlanCheckoutKey(crypto.randomUUID()) } finally { setBusy(false) }
  }
  ...
  return <Page title="Escolha como ativar sua conta" subtitle="Seu acesso foi criado. Agora selecione uma das duas modalidades para iniciar sua participação.">
    ...
    <section className="activation-options" aria-label="Modalidades de ativação">
      ...
      <article className="activation-card featured">
        ...
        <ul><li><Check aria-hidden="true" />Sem pagamento adicional do plano</li><li><Check aria-hidden="true" />Teto de até 250% total por cota (150% de rendimento além do valor investido)</li></ul>
...
function Store({ session }: { session: Session }) {
  ...
  const buy = async (product: typeof products[number]) => {
    ...
    try { await api.post('/wallet/purchases', { productType: 'PRODUCT', productId: product.id, idempotencyKey: purchaseKeys.current[product.id] ??= crypto.randomUUID() }); delete purchaseKeys.current[product.id]; setNotice(`${product.name} adicionado aos seus pedidos.`); await load() }
...
function UserFinance({ session }: { session: Session }) {
  ...
  const confirmDemoDeposit = async () => {
    if (!deposit) return
    setBusy(true); setActionError('')
    try { await api.post(`/deposits/${deposit.id}/confirm-demo`, {}); setDeposit(undefined); setDepositKey(crypto.randomUUID()); await load(); setNotice('Depósito de demonstração creditado na Carteira de Saldo.') }
    catch (reason: any) { setActionError(reason.message) } finally { setBusy(false) }
  }
  if (!data && !error) return <Loader />
  const state = data || emptyState
...
function Profile({ session }: { session: Session }) {
  ...
  return <Page title="Meu perfil" subtitle="Mantenha seus dados e preferências de segurança atualizados."><ErrorBox error={error || actionError} />{saved && <div className="success-box" role="status"><Check aria-hidden="true" />Perfil salvo com sucesso.</div>}<form className="form-panel profile-form" onSubmit={submit} aria-busy={busy}><div className="profile-identity"><span className="avatar large">{initials(form?.name || session.user.name)}</span><div><h2>{form?.name || session.user.name}</h2><p>@{session.user.username}</p></div></div><div className="form-grid"><label>Nome completo<input autoComplete="name" value={form?.name || ''} onChange={event => update('name', event.target.value)} /></label><label>E-mail<input autoComplete="email" type="email" value={form?.email || ''} onChange={event => update('email', event.target.value)} /></label><label>CPF (titular para saques PIX)<input inputMode="numeric" autoComplete="off" pattern="[0-9]{11}" maxLength={11} title="Informe os 11 dígitos do CPF, somente números" placeholder="Somente números (11 dígitos)" value={form?.cpf || ''} onChange={event => update('cpf', event.target.value)} /></label><label>Telefone<input autoComplete="tel" value={form?.phone || ''} onChange={event => update('phone', event.target.value)} /></label><label>País<input autoComplete="country-name" value={form?.country || ''} onChange={event => update('country', event.target.value)} /></label></div><div className="toggle-row"><span><b>2FA no login</b><small>Proteção adicional para acessar a conta</small></span><input aria-label="Ativar 2FA no login" type="checkbox" checked={!!form?.twoFactorLogin} onChange={event => update('twoFactorLogin', event.target.checked)} /></div><div className="toggle-row"><span><b>2FA nos saques</b><small>Confirmação extra para movimentações</small></span><input aria-label="Ativar 2FA nos saques" type="checkbox" checked={!!form?.twoFactorWithdraw} onChange={event => update('twoFactorWithdraw', event.target.checked)} /></div><button className="primary-btn" disabled={busy}>{busy ? 'Salvando…' : 'Salvar alterações'}</button></form></Page>
}

function UserTable({ users, onSelect, onAccess, onAccount, showCpf }: { users: User[]; onSelect?: (user: User) => void; onAccess?: (user:User)=>void; onAccount?: (user:User)=>void; showCpf?: boolean }) {
...
function AccountManagement({session,account,close}:{session:Session;account:User;close:()=>void}) {
 ...
 const load=()=>api.get(`/admin/associates/${account.id}/account`).then(setData)
...
function AdminDashboard({ session }: { session: Session }) {
  ...
  useEffect(() => { void load() }, [load])
  useAutoRefresh(load)
  return <Page title="Central MASTER" subtitle="Aderência operacional ao Plano de Negócios GoMove."><ErrorBox error={error} />{data ? <>{data.control && <><h2>Controle diário e total</h2><p>Hoje: {data.control.day.split('-').reverse().join('/')} · Horário de São Paulo. Pessoas = participantes, sem administradores.</p><section className="metric-grid admin-grid">
<Metric label="CADASTROS / PESSOAS HOJE" value={String(data.control.registrationsToday)} icon={UsersRound} note="Novos participantes no dia" />
<Metric label="CADASTROS / PESSOAS TOTAL" value={String(data.control.peopleTotal)} icon={UsersRound} note="Todos os participantes cadastrados" />
<Metric label="CADASTROS ATIVOS HOJE" value={String(data.control.activeRegistrationsToday)} icon={ShieldCheck} note="Cadastrados hoje com conta ativa" />
<Metric label="PESSOAS ATIVAS TOTAL" value={String(data.control.activePeopleTotal)} icon={ShieldCheck} note="Contas atualmente ativas" />
<Metric label="ENTRADAS HOJE" value={cents(data.control.cashInTodayCents)} icon={Wallet} note="Pagamentos externos recebidos no dia" />
...
function Associates({ session }: { session: Session }) {
  ...
  const [form, setForm] = useState({ name: '', email: '', username: '', phone: '', cpf: '', password: '', sponsorId: '__MASTER__', status: 'PENDING', associatePlanStatus: 'PENDING' })
  const [error, setError] = useState('')
  const [notice, setNotice] = useState('')
  const [busy, setBusy] = useState(false)
  const load = async () => { try { const accounts:User[]=[];let page=1,total=0;do { const result=await api.get<ApiPage<User>>(`/admin/associates?pageSize=100&page=${page++}`);accounts.push(...result.items);total=result.total;if(!result.items.length)break } while(accounts.length<total);setRows(accounts) } catch(reason:any){setError(reason.message)} }
  useEffect(() => { void load() }, [])
  const openCreate = () => { setSelected(undefined); setCreating(true); setForm({ name: '', email: '', username: '', phone: '', cpf: '', password: '', sponsorId: '__MASTER__', status: 'PENDING', associatePlanStatus: 'PENDING' }); setError('') }
  const openEdit = (account: User) => { setCreating(false); setSelected(account); setForm({ name: account.name, email: account.email || '', username: account.username, phone: (account as Row).phone || '', cpf: (account as Row).cpf || '', password: '', sponsorId: rows.some(item => item.id === account.sponsorId) ? account.sponsorId || '__MASTER__' : '__MASTER__', status: account.status, associatePlanStatus: account.associatePlanStatus || 'PENDING' }); setError('') }
