...
function Commissions({ session }: { session: Session }) {
  ...
  const [ruleForm, setRuleForm] = useState({ name: '', levels: '1:10, 2:5, 3:3', active: false })
  const [editingRule, setEditingRule] = useState<CommissionRule>()
  const [ruleEditor, setRuleEditor] = useState(false)
  const [error, setError] = useState('')
  const [notice, setNotice] = useState('')
  const [busy, setBusy] = useState(false)
  const load = () => Promise.all([api.get<ApiPage<CommissionRule>>('/admin/commission-rules?pageSize=100'), api.get<ApiPage<Bonus>>('/admin/bonus-entries?pageSize=100'), api.get<ApiPage<User>>('/admin/associates?pageSize=100')]).then(([a, b, c]) => { setRules(a.items); setBonuses(b.items); setUsers(c.items) }).catch(reason => setError(reason.message))
  useEffect(() => { void load() }, [])
  ...
  return <Page title="Comissões e bônus" subtitle="Configure níveis unilevel, confirme lançamentos e acompanhe o crédito de cada usuário." action={<button className="primary-btn" onClick={() => openRule()}><Plus aria-hidden="true" />Nova regra</button>}>
    ...
    <div className="panel rules-panel"><div className="panel-title"><h2>Regras de comissionamento</h2><span>{rules.length} cadastradas</span></div>{rules.length ? rules.map(rule => <div className="rule-row commission-rule-row" key={rule.id}><span><b>{rule.name}</b><small>{rule.levels.map(level => `N${level.level}: ${level.bps / 100}%`).join(' · ')}</small></span>{status(rule.active ? 'ACTIVE' : 'INACTIVE')}<div className="row-actions"><button className="outline-btn" disabled={busy} onClick={() => void activateRule(rule)}>{rule.active ? 'Desativar' : 'Ativar'}</button><button className="icon-btn" aria-label={`Editar regra ${rule.name}`} onClick={() => openRule(rule)}><Pencil aria-hidden="true" /></button><button className="icon-btn danger-icon" aria-label={`Excluir regra ${rule.name}`} disabled={rule.active || busy} onClick={() => void removeRule(rule)}><Trash2 aria-hidden="true" /></button></div></div>) : <div className="empty-panel"><Activity aria-hidden="true" /><h2>Nenhuma regra cadastrada</h2><p>Crie e ative uma regra antes de confirmar investimentos.</p></div>}</div>
    <form className="form-panel commission-credit-form" onSubmit={manual} aria-busy={busy}><h2>Novo crédito manual</h2><p>O valor será lançado como pendente e só entrará no saldo depois da aprovação.</p><div className="form-grid"><label>Usuário ativo<select required value={form.userId} onChange={event => setForm({ ...form, userId: event.target.value })}><option value="">Selecione o usuário</option>{users.filter(user => user.status === 'ACTIVE').map(user => <option value={user.id} key={user.id}>{user.name}</option>)}</select></label><label>Valor em reais<input required min="0.01" step="0.01" type="number" value={form.amount} onChange={event => setForm({ ...form, amount: event.target.value })} /></label><label className="wide">Justificativa<input required value={form.reason} onChange={event => setForm({ ...form, reason: event.target.value })} /></label></div><button className="primary-btn" disabled={busy}>Criar crédito pendente</button></form>
    <h2 className="section-title">Lançamentos</h2><DataTable rows={bonusRows as Row[]} columns={[["ownerName", "USUÁRIO"], ["type", "TIPO"], ["level", "NÍVEL", row => row.level ? `N${row.level}` : '—'], ["amountCents", "VALOR", row => cents(row.amountCents)], ["reason", "MOTIVO"], ["status", "STATUS", row => status(row.status)]]} action={row => row.status === 'PENDING' ? <div className="inline-actions"><button disabled={busy} onClick={() => void transition(row as Bonus, 'approve')}>Aprovar</button><button disabled={busy} onClick={() => void transition(row as Bonus, 'cancel')}>Cancelar</button></div> : row.status === 'APPROVED' && row.type !== 'REVERSAL' ? <button className="outline-btn" disabled={busy} onClick={() => void transition(row as Bonus, 'reverse')}>Estornar</button> : <span>—</span>} />
    {ruleEditor && <Modal title={editingRule ? 'Editar regra de comissão' : 'Nova regra de comissão'} close={() => !busy && setRuleEditor(false)}><form className="modal-form" onSubmit={saveRule} aria-busy={busy}><label>Nome da regra<input required value={ruleForm.name} onChange={event => setRuleForm({ ...ruleForm, name: event.target.value })} /></label><label>Níveis e percentuais<input required value={ruleForm.levels} onChange={event => setRuleForm({ ...ruleForm, levels: event.target.value })} placeholder="1:10, 2:5, 3:3" /><small>Use nível:percentual separados por vírgula. Ex.: 1:10, 2:5, 3:3.</small></label><label className="check"><input type="checkbox" checked={ruleForm.active} onChange={event => setRuleForm({ ...ruleForm, active: event.target.checked })} />Ativar esta regra após salvar</label><div className="modal-actions"><button type="button" className="outline-btn" disabled={busy} onClick={() => setRuleEditor(false)}>Cancelar</button><button className="primary-btn" disabled={busy}>{busy ? 'Salvando…' : 'Salvar regra'}</button></div></form></Modal>}
  </Page>
}
