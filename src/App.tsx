...
function UserFinance({ session }: { session: Session }) {
  ...
  const withdraw = async (event: FormEvent) => {
    ...
    try { await api.post('/withdrawals', { amount: Number(amount), account: data?.profile.pixType || 'CPF' }); setAmount(''); setNotice('Solicitação de saque enviada para análise.'); await load() }
    catch (reason: any) { setActionError(reason.message) } finally { setBusy(false) }
  }
  ...
  if (!data && !error) return <Loader />
  const state = data || emptyState
  const wallets = state.business.wallets || { balanceCents: 0, earningsCents: 0, withdrawableCents: 0, reservedCents: 0, hasActivePackage: false }
  ...
  return <Page title="Carteiras e financeiro" subtitle="Depósitos para compras. Rendimentos para saques com pacote ativo.">
    ...
    <section className="metric-grid wallet-metrics">
      ...
      <Metric label="CARTEIRA DE RENDIMENTOS" value={cents(wallets.earningsCents)} icon={CircleDollarSign} note="Indicações, bonificações, rendimentos e demais ganhos." />
      <Metric label="DISPONÍVEL PARA SAQUE" value={cents(wallets.withdrawableCents)} icon={WalletCards} note={`${cents(wallets.reservedCents)} reservados em solicitações pendentes.`} />
    </section>
    <section className="dashboard-split wallet-forms">
      ...
      <form className="form-panel" onSubmit={withdraw} aria-busy={busy}>
        <h2>Solicitar saque</h2><p>Somente valores da Carteira de Rendimentos podem ser sacados. É obrigatório ter um pacote ativo.</p>
        {!wallets.hasActivePackage && <p role="status">Saque bloqueado: adquira ou ative um pacote para sacar seus rendimentos.</p>}
        <label>Valor do saque (R$)<input required min="50" max={wallets.withdrawableCents / 100} step="0.01" type="number" inputMode="decimal" value={amount} onChange={event => setAmount(event.target.value)} placeholder="R$ 0,00" disabled={!wallets.hasActivePackage} /></label>
        <p>Disponível: {cents(wallets.withdrawableCents)}. Mínimo: R$ 50,00. O pedido será revisado pelo financeiro MASTER.</p>
        <button className="primary-btn" disabled={busy || wallets.withdrawableCents < 5000}>{busy ? 'Enviando…' : 'Enviar solicitação'}</button>
      </form>
    </section>
...
function AccountManagement({session,account,close}:{session:Session;account:User;close:()=>void}) {
 ...
 const [adjustmentWallet,setAdjustmentWallet]=useState<'BALANCE'|'EARNINGS'>('BALANCE')
