...
function UserInvestments({ session }: { session: Session }) {
  ...
  const openCheckout = (event: FormEvent) => {
    event.preventDefault(); setActionError(''); setCheckoutResult(undefined); setPaymentOption('BTC'); setCheckoutKey(crypto.randomUUID()); setCheckoutOpen(true)
  }
...
function Commissions({ session }: { session: Session }) {
  ...
  const openRule = (rule?: CommissionRule) => { setEditingRule(rule); setRuleForm(rule ? { name: rule.name, directReferralPercent: String((rule.directReferralBps ?? DIRECT_REFERRAL_BPS) / 100), levels: rule.levels.map(item => `${item.level}:${item.bps / 100}`).join(', '), active: rule.active } : { name: 'Indicação direta + Unilevel GoMove', directReferralPercent: String(DIRECT_REFERRAL_BPS / 100), levels: defaultUnilevelText, active: false }); setRuleEditor(true); setError('') }
