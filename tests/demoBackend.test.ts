...
test('approved bonuses fund withdrawals and paid withdrawals debit the user ledger once', async () => {
  ...
  await assert.rejects(() => demoRequest('/withdrawals', 'POST', { amount: 1_000, method: 'PIX', account: '12345678901' }, user.token), /Valor indisponível/)
  // A Carteira Rede não tem limite de quantidade: só o saldo e o mínimo de R$ 55 limitam.
  const sameDay = await demoRequest<Record<string, any>>('/withdrawals', 'POST', { amount: 55, wallet: 'REDE', method: 'PIX' }, user.token)
  assert.equal(sameDay.status, 'Pendente')
  assert.equal(sameDay.feeCents, 330)
  assert.equal(sameDay.netCents, 5170)
  await assert.rejects(() => demoRequest('/withdrawals', 'POST', { amount: 150, wallet: 'REDE', method: 'PIX' }, user.token), /Valor indisponível/)
...
test('invited account enters immediately and keeps participation pending until choosing a product', async () => {
  localStorage.clear()
  const registration = await demoRequest<{ token: string; user: User }>('/public/register', 'POST', { name: 'Nova Pessoa', email: 'nova@gomove.com.br', username: 'nova', password: 'segura123', cpf: '99000010284', inviteCode: 'matheus01' })
...
test('direct registration uses MASTER sponsorship and can acquire quotas without the associate plan', async () => {
  localStorage.clear()
  const registration = await demoRequest<{ token: string; user: User }>('/public/register', 'POST', { name: 'Cotista Direto', email: 'cotista.direto@gomove.com.br', username: 'cotistadireto', password: 'segura123', cpf: '99000010365' })
  assert.equal(registration.user.status, 'ACTIVE')
  assert.equal(registration.user.associatePlanStatus, 'PENDING')
...
test('associate choice creates one R$ 55 checkout per idempotency key', async () => {
  localStorage.clear()
  const registration = await demoRequest<{ token: string }>('/public/register', 'POST', { name: 'Associado Novo', email: 'associado.novo@gomove.com.br', username: 'associadonovo', password: 'segura123', cpf: '99000010446' })
...
test('demo admin follows the same activation, sponsor and quota safety rules as production', async () => {
  ...
  const unpaid = await demoRequest<User>('/admin/associates', 'POST', {
    ...
    password: 'senha-segura', cpf: '99000010608', sponsorId: master.user.id, status: 'ACTIVE', associatePlanStatus: 'PENDING',
  }, master.token)
...
test('demo registration and quota inputs reject malformed credentials and unsafe money', async () => {
  ...
  const registration = await demoRequest<any>('/public/register', 'POST', { name: 'Cota Segura', username: `safe-${suffix}`, email: `safe-${suffix}@gomove.local`, password: 'senha-segura', cpf: '99000010799' })
  await assert.rejects(
...
test('registration requires a valid unique CPF and profile updates follow the same rule', async () => {
  ...
  await assert.rejects(() => demoRequest('/public/register', 'POST', { name: 'Cpf Checksum', username: `cpf-d-${suffix}`, email: `cpf-d-${suffix}@gomove.local`, password: 'senha-segura', cpf: '12345678900' }), /CPF inválido/)
  ...
  await demoRequest('/profile', 'PUT', { cpf: '529.982.247-25' }, userA.token)
  const updated = await demoRequest<Record<string, any>>('/state', 'GET', undefined, userA.token)
  assert.equal(updated.profile.cpf, '52998224725')
})

test('payout key must match the CPF collected at registration', async () => {
  localStorage.clear()
  const master = await demoRequest<{ token: string; user: User }>('/auth/login', 'POST', { username: 'admin', password: 'gomove2026' })
  const registration = await demoRequest<{ token: string; user: User }>('/public/register', 'POST', { name: 'Titular PIX', username: 'titularpix', email: 'titular.pix@gomove.com.br', password: 'segura123', cpf: '99000020247' })
